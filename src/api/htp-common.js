...
import { DOMAIN, API_VERSION, LOCAL_STORAGE_KEY } from "@/core/constants/constants";
        ...
        callApiLayLaiToken()
          ...
          .catch((err) => {
            if (err && err.response.status === 406) {
              ...
              localStorage.clear();
              if (myOPT)
                localStorage.setItem('otp', myOPT);
              if (old_url_redirect)
                localStorage.setItem('old_url_redirect', old_url_redirect);
